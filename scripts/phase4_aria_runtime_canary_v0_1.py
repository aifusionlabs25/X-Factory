#!/usr/bin/env python3
"""One-transaction Aria continuation controller for the accepted Atlas mission.
...
ACTIVATION_ID = "phase4-aria-canary-001"
MISSION_ID = "phase4-atlas-canary-003"
STAGE = "ARIA_BLUEPRINT"
PACKET = ROOT / "approvals/pending" / ACTIVATION_ID
ACTIVATION_MANIFEST = PACKET / "activation-manifest.json"
BASE_MANIFEST = ROOT / "approvals/pending/phase4-live-synthetic-001-revision-006/live-mission-manifest.v0.3.json"
APPROVAL_RECORD = ROOT / "approvals/approved/phase4-aria-canary-001.activation.approval.json"
...
def verify_approval(activation: dict) -> None:
    if not APPROVAL_RECORD.is_file():
        raise PermissionError("Exact Aria continuation approval record is absent")
    approval = load(APPROVAL_RECORD)
    if (
        approval.get("status") != "APPROVED_ONE_ARIA_CONTINUATION"
        ...
        raise PermissionError("Aria continuation approval record mismatch")
    if approval.get("authority") != activation.get("authority"):
        raise PermissionError("Aria continuation authority mismatch")


def build_bound_runner(activation: dict) -> DerivationBoundRunnerV07:
    schema_path = ROOT / "contracts/phase4/aria_runtime_continuation_activation.v0.1.schema.json"
    if file_hash(schema_path) != activation["activation_schema_sha256"]:
        raise PermissionError("Aria activation schema drift")
    ...
    if file_hash(Path(__file__).resolve()) != activation["controller_sha256"]:
        raise PermissionError("Aria controller drift")
    ...
    if ledger_document.get("mission_id") != MISSION_ID or not isinstance(entries, list) or len(entries) != 1:
        raise PermissionError("Exactly one accepted ledger prefix is required")
    if entries[0].get("transaction") != 1 or entries[0].get("stage") != "ATLAS_TRIAGE" or entries[0].get("status") != "ACCEPTED":
        raise PermissionError("Accepted Atlas ledger prefix is invalid")

    manifest = load(BASE_MANIFEST)
    manifest["mission_id"] = MISSION_ID
    manifest["purpose"] = "Continue the accepted Atlas result through exactly one contained Aria blueprint transaction."
    ...
    if Path(boundary["aria_profile_root"]).resolve() != (HERMES / "profiles/aria").resolve():
        raise PermissionError("Declared Aria profile root mismatch")
    ...
    if sha256_bytes(canonical(payload)) != activation["payload_sha256"]:
        raise PermissionError("Derived Aria payload mismatch")
    if sha256_bytes(prompt.encode("utf-8")) != activation["prompt_sha256"]:
        raise PermissionError("Derived Aria prompt mismatch")
    if file_hash(ROOT / "contracts/x_agent_blueprint.v0.1.schema.json") != activation["output_schema_sha256"]:
        raise PermissionError("Aria output schema drift")
    return runner
...
def main() -> None:
    ...
    print(json.dumps({"result": "ARIA_CANARY_ACCEPTED", "mission_id": MISSION_ID, "output": output}, indent=2))
