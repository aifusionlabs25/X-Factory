...
def stage_prompt(role: str, payload: dict[str, Any], schema: dict[str, Any]) -> str:
    ...
    if role == "atlas":
        ...
        instruction = (
            ...
            "Treat the three supplied Phase 1.1 closure contracts as exact authoritative implementation-handoff requirements, resolve the prior gaps directly from them, "
        ...
        instruction = (
            ...
            "Also evaluate it against every exact Phase 1.1 closure contract. Do not rewrite the blueprint. Return the strictest evidence-supported verdict."
        )
...
def main() -> int:
    ...
    approval_schema = load_json(root / "contracts" / "component_card_approval.v0.2.schema.json")
    disclosure_schema = load_json(root / "contracts" / "disclosure_manifest.v0.2.schema.json")
    ...
    run_schema = load_json(root / "contracts" / "contained_run_record.v0.2.schema.json")
    closure_dir = root / "contracts" / "phase1_1"
    closure_specs = {
        "output_bundle_manifest": ("output_bundle_manifest.v0.1.json", "output_bundle_manifest.v0.1.schema.json"),
        "deterministic_generation": ("deterministic_generation.v0.1.json", "deterministic_generation.v0.1.schema.json"),
        "acceptance_fixture": ("acceptance_fixture.v0.1.json", "acceptance_fixture.v0.1.schema.json"),
    }
    ...
    disclosure_manifest = {
        "schema_version": "0.2",
        "manifest_id": "DISCLOSURE-DETERMINISTIC-BUNDLE-GENERATOR-V2",
        "approval_id": "APPROVAL-DETERMINISTIC-BUNDLE-GENERATOR-V2",
        "purpose": "Prove the sanitized component card and exact Phase 1.1 closure contracts can produce a complete evidence-bounded blueprint",
        ...
        "scope": "EXACT_CARD_CLOSURE_CONTRACTS_AND_MODEL_GENERATED_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    manifest_path = disclosure_dir / f"{card['card_id']}.phase1-1.{card_hash[-12:]}.disclosure-manifest.json"
    ...
    preview = {
        ...
        "closure_contract_hashes": closure_hashes,
        "allowed_payload_scope": "EXACT_CARD_CLOSURE_CONTRACTS_AND_MODEL_GENERATED_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    if approval["closure_contract_hashes"] != closure_hashes:
        raise PermissionError("Approval does not match the exact Phase 1.1 closure contract bytes")
    ...
    write_json(run_dir / "contained_run_record.v0.2.json", record)
