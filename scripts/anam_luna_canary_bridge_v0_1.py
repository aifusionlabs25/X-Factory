#!/usr/bin/env python3
"""One-call, zero-tool Hermes bridge for the contained Mia/Luna canary."""
...
from x_factory.phase4_transport_guard_v0_4 import install_into_hermes  # noqa: E402
...
def main() -> int:
    ...
    execution_root.relative_to((ROOT / "canaries" / "anam").resolve())
    prompt_file = args.prompt_file.resolve(strict=True)
    evidence_file = args.evidence_file.resolve(strict=False)
    prompt_file.relative_to(execution_root)
    evidence_file.relative_to(execution_root)
    ...
    error_message = None
    sys.argv = [
        "hermes", "--profile", "aria", "chat", "-Q",
        "--provider", "openai-codex", "--model", "gpt-5.6-luna",
        "--reasoning", "low", "--max-turns", "1", "--source", "tool",
        "--in", str(execution_root), "-q", prompt_bytes.decode("utf-8"),
    ]
    try:
