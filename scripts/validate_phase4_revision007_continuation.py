#!/usr/bin/env python3
"""Read-only accepted Atlas-and-Aria to Vera continuation proof."""
...
PACKET = ROOT / "approvals/pending/phase4-live-synthetic-001-revision-007"
...
manifest["purpose"] = "Continue accepted Atlas and Aria outputs into one disclosure-only Vera specification-review preview."
manifest["starting_input_hashes"] = {
    ...
    "evidence_index": sha256_bytes(EVIDENCE_INDEX.read_bytes()),
}
...
def restore(stage: str, evidence_path: Path) -> dict:
    execution = load(evidence_path)
    output = json.loads(base64.b64decode(execution["response_base64"], validate=True).decode("utf-8"))
    runner.restore_accepted_output(stage, output, evidence_path)
    return output


atlas_output = restore("ATLAS_TRIAGE", ATLAS_EVIDENCE)
aria_output = restore("ARIA_BLUEPRINT", ARIA_EVIDENCE)
payload, prompt = runner.compose_transmission("VERA_SPEC_REVIEW")

(PACKET / "vera-derived-payload.json").write_bytes(canonical(payload))
(PACKET / "vera-derived-prompt.txt").write_text(prompt, encoding="utf-8", newline="")

print(json.dumps({
    "result": "PASS_DISCLOSURE_ONLY",
    "mission_id": MISSION_ID,
    "accepted_prefix_transactions": len(runner.chain),
    "restored_atlas_output_sha256": sha256_bytes(canonical(atlas_output)),
    "atlas_execution_evidence_sha256": sha256_bytes(ATLAS_EVIDENCE.read_bytes()),
    "restored_aria_output_sha256": sha256_bytes(canonical(aria_output)),
    "aria_execution_evidence_sha256": sha256_bytes(ARIA_EVIDENCE.read_bytes()),
    "accepted_ledger_sha256": sha256_bytes((MISSION_ROOT / "control/transaction-ledger.json").read_bytes()),
    "evidence_index_file_sha256": sha256_bytes(EVIDENCE_INDEX.read_bytes()),
    "vera_payload_sha256": sha256_bytes(canonical(payload)),
    "vera_prompt_sha256": sha256_bytes(prompt.encode("utf-8")),
    "vera_output_schema_sha256": sha256_bytes((ROOT / "contracts/evaluation_contract.v0.1.schema.json").read_bytes()),
    "ledger_writes": 0,
    "profile_writes": 0,
