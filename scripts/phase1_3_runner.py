...
def run_profile(profile: str, reasoning: str, prompt: str, factory_root: Path) -> tuple[dict[str, Any], str, str]:
    ...
    bridge = factory_root / "scripts" / "hermes_prompt_bridge.py"
    ...
    try:
        with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", suffix=".txt", prefix="hermes-approved-prompt-", delete=False) as handle:
            handle.write(prompt)
            prompt_path = Path(handle.name)
        command = [
            str(hermes_python), str(bridge),
            "--profile", profile,
            "--reasoning", reasoning,
...
def stage_prompt(role: str, payload: dict[str, Any], schema: dict[str, Any]) -> str:
    ...
    if role == "atlas":
        ...
        instruction = (
            ...
            "Treat the three supplied Phase 1.3 byte-bound interface contracts as exact authoritative implementation-handoff requirements, resolve the prior gaps directly from them, "
            "set specification_status to SPECIFICATION_READY_FOR_BUILD, and keep every authority boolean false."
        )
    else:
        instruction = (
            ...
            "Also evaluate it against every exact Phase 1.3 interface contract, including the unified fixture-root path model, embedded document schemas, template source hashes, template-set hash, manifest self-reference rule, and value-origin precedence. The blueprint schema requires a top-level requirements array; inspect that field directly. Do not rewrite the blueprint. Return the strictest evidence-supported verdict."
        )
...
def main() -> int:
    ...
    approval_schema = load_json(root / "contracts" / "component_card_approval.v0.4.schema.json")
    disclosure_schema = load_json(root / "contracts" / "disclosure_manifest.v0.4.schema.json")
    ...
    run_schema = load_json(root / "contracts" / "contained_run_record.v0.4.schema.json")
    closure_dir = root / "contracts" / "phase1_3"
    closure_specs = {
        "output_bundle_manifest": ("output_bundle_manifest.v0.3.json", "output_bundle_manifest.v0.3.schema.json"),
        "deterministic_generation": ("deterministic_generation.v0.3.json", "deterministic_generation.v0.3.schema.json"),
        "acceptance_fixture": ("acceptance_fixture.v0.3.json", "acceptance_fixture.v0.3.schema.json"),
    }
    closure_contracts: dict[str, Any] = {}
    closure_hashes: dict[str, str] = {}
    for closure_name, (instance_name, schema_name) in closure_specs.items():
        instance_path = closure_dir / instance_name
        closure_contracts[closure_name] = load_json(instance_path)
        validate(closure_contracts[closure_name], load_json(closure_dir / schema_name), f"{closure_name} closure contract")
        closure_hashes[closure_name] = sha256_file(instance_path)
    ...
    disclosure_manifest = {
        "schema_version": "0.4",
        "manifest_id": "DISCLOSURE-DETERMINISTIC-BUNDLE-GENERATOR-V4",
        "approval_id": "APPROVAL-DETERMINISTIC-BUNDLE-GENERATOR-V4",
        "purpose": "Prove the sanitized component card and exact byte-bound Phase 1.3 contracts can produce a build-complete evidence-bounded blueprint",
        ...
        "closure_contract_hashes": closure_hashes,
        "provider": PROVIDER,
        "model": MODEL,
        "profiles": list(PROFILES),
        "profile_config_hashes": profile_hashes,
        "scope": "EXACT_CARD_PHASE1_3_BYTE_BOUND_CONTRACTS_AND_MODEL_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    manifest_path = disclosure_dir / f"{card['card_id']}.phase1-3.{card_hash[-12:]}.disclosure-manifest.json"
    ...
    preview = {
        ...
        "exact_closure_contracts": closure_contracts,
        "closure_contract_hashes": closure_hashes,
        "allowed_payload_scope": "EXACT_CARD_PHASE1_3_BYTE_BOUND_CONTRACTS_AND_MODEL_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    if approval["closure_contract_hashes"] != closure_hashes:
        raise PermissionError("Approval does not match the exact Phase 1.3 contract bytes")
    ...
    aria_raw_path = run_dir / "aria" / "raw-response.txt"
    if run_dir.exists():
        existing_files = {path.relative_to(run_dir).as_posix() for path in run_dir.rglob("*") if path.is_file()}
        resumable_states = (
            {"atlas/intake.v0.1.json", "atlas/raw-response.txt"},
            {"atlas/intake.v0.1.json", "atlas/raw-response.txt", "aria/x_agent_blueprint.v0.1.json", "aria/raw-response.txt"},
        )
        if existing_files not in resumable_states:
            raise FileExistsError(f"Contained run directory is not an exact resumable partial run: {run_dir}")
    ...
    governed = {"governed_run_marker": "X_FACTORY_GOVERNED_RUN_V0_1", "execution_mode": EXECUTION_MODE, "disclosure_manifest_hash": manifest_hash, "approval_hash": approval_hash, "closure_contract_hashes": closure_hashes}
    if atlas_intake_path.is_file() and atlas_raw_path.is_file():
        ...
        atlas_payload = {**governed, "card": card, "intake": intake, "closure_contracts": closure_contracts}
    ...
    if blueprint_path.is_file() and aria_raw_path.is_file():
        blueprint = load_json(blueprint_path)
        aria_raw = aria_raw_path.read_text(encoding="utf-8")
        aria_session = extract_session_id(aria_raw)
        aria_tool_calls = session_tool_count("aria", aria_session, root)
        validate(blueprint, blueprint_schema, "Resumed Aria blueprint")
    else:
        aria_payload = {**governed, "card": card, "normalized_intake": atlas, "closure_contracts": closure_contracts}
        ...
        aria_raw_path.write_text(aria_raw, encoding="utf-8")
    blueprint_hash = sha256_file(blueprint_path)

    vera_payload = {**governed, "card": card, "normalized_intake": atlas, "blueprint": blueprint, "blueprint_hash": blueprint_hash, "closure_contracts": closure_contracts}
    ...
    write_json(run_dir / "contained_run_record.v0.4.json", record)
