...
def stage_prompt(role: str, payload: dict[str, Any], schema: dict[str, Any]) -> str:
    ...
    if role == "atlas":
        ...
        instruction = (
            ...
            "Treat the three supplied Phase 1.2 interface contracts as exact authoritative implementation-handoff requirements, resolve the prior gaps directly from them, "
        ...
        instruction = (
            ...
            "Also evaluate it against every exact Phase 1.2 interface contract. The blueprint schema requires a top-level requirements array; inspect that field directly and do not confuse it with explicit_requirements or raw_intake. Do not rewrite the blueprint. Return the strictest evidence-supported verdict."
        )
...
def main() -> int:
    ...
    approval_schema = load_json(root / "contracts" / "component_card_approval.v0.3.schema.json")
    disclosure_schema = load_json(root / "contracts" / "disclosure_manifest.v0.3.schema.json")
    ...
    run_schema = load_json(root / "contracts" / "contained_run_record.v0.3.schema.json")
    closure_dir = root / "contracts" / "phase1_2"
    closure_specs = {
        "output_bundle_manifest": ("output_bundle_manifest.v0.2.json", "output_bundle_manifest.v0.2.schema.json"),
        "deterministic_generation": ("deterministic_generation.v0.2.json", "deterministic_generation.v0.2.schema.json"),
        "acceptance_fixture": ("acceptance_fixture.v0.2.json", "acceptance_fixture.v0.2.schema.json"),
    }
    ...
    disclosure_manifest = {
        "schema_version": "0.3",
        "manifest_id": "DISCLOSURE-DETERMINISTIC-BUNDLE-GENERATOR-V3",
        "approval_id": "APPROVAL-DETERMINISTIC-BUNDLE-GENERATOR-V3",
        "purpose": "Prove the sanitized component card and exact Phase 1.2 interface contracts can produce a complete evidence-bounded blueprint",
        ...
        "scope": "EXACT_CARD_PHASE1_2_INTERFACE_CONTRACTS_AND_MODEL_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    manifest_path = disclosure_dir / f"{card['card_id']}.phase1-2.{card_hash[-12:]}.disclosure-manifest.json"
    ...
    preview = {
        ...
        "closure_contract_hashes": closure_hashes,
        "allowed_payload_scope": "EXACT_CARD_PHASE1_2_INTERFACE_CONTRACTS_AND_MODEL_DERIVATIVES_FOR_ONE_CONTAINED_ATLAS_ARIA_VERA_PROOF",
    ...
    if approval["closure_contract_hashes"] != closure_hashes:
        raise PermissionError("Approval does not match the exact Phase 1.2 interface contract bytes")
    ...
    write_json(run_dir / "contained_run_record.v0.3.json", record)
