...
For an owner-visible smoke test, append `?mode=ephemeral-test` to the hosted URL. That mode runs the Prepare → Review → Approve → Build → Test → Dojo rehearsal in browser memory only. The exact candidate is carried into the generated app link so a new tab can be tested. Refreshing the tab resets the rehearsal; no Vercel, GitHub, ANAM, Hermes, provider, or local-repository writes occur. It is a visual/e2e rehearsal, not a replacement for the local Factory's persistent workflow.
