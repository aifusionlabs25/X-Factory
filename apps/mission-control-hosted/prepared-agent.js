...
(() => {
  ...
  function render(project) {
    ...
    if(['RESEARCHING','PREPARING','BUILDING'].includes(project.status)){
      ...
      const open=node('a',`Open ${project.fields.x_agent_name||'your agent'}’s app ↗`,guide);open.className='prepared-open-candidate';open.href=`/agent-app.html?project=${encodeURIComponent(project.project_id)}`;open.target='_blank';open.rel='noopener';
