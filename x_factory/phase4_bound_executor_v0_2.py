...
class BoundStageExecutorV02:
    ...
    def execute_stage(self, stage: str) -> dict[str, Any]:
        ...
        try:
            ...
            if execution.get("extraction_error") is not None:
                raise PermissionError("Bound bridge response extraction failed")
            ...
            if sha256_bytes(response_bytes) != execution.get("response_sha256"):
                raise PermissionError("Extracted response hash mismatch")
            output = json.loads(response_bytes.decode("utf-8"))
