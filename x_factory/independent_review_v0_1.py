...
def run_independent_review(mission_id: str, mission_root: Path | None = None, review_root: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        build_route = [item.get("specialist") for item in specialists[:5]]
        expected_build_route = ["Atlas", "Aria", "Vera", "Mason", "Vera"]
        _check(checks, "FACTORY_ROUTE", len(specialists) >= 5 and build_route == expected_build_route and all(item.get("status") == "PASS" for item in specialists), "The certified build route passed before the independent review and packaging gates")
        ...
        review = {
            ...
            "reviewer": {"name": "Rook", "mode": "INDEPENDENT_PROVIDER_FREE_REVIEW", "independent_from": ["Atlas", "Aria", "Vera", "Mason", "Porter"], "provider_calls": 0},
